...
check-and-fund:
	...
	balance=$$(curl -s "https://api-mocha.pops.one/cosmos/bank/v1beta1/balances/$$address" | jq -r '.balances[] | select(.denom == "utia") | .amount // "0"'); \
	if [[ $$balance =~ ^[0-9]+$$ ]]; then \
...
reset-node:
	...
	@block_response=$$(curl -s https://rpc-mocha.pops.one/block); \
