...
func TestGetShare(t *testing.T) {
	...
	shares := share.RandShares(size * size)
...
func TestBlockRecovery(t *testing.T) {
	...
	// generate test data
	quarterShares := share.RandShares(shareCount)
	allShares := share.RandShares(shareCount)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			reds, err := rsmt2d.ImportExtendedDataSquare(rdata, square.DefaultRSMT2DCodec(), wrapper.NewConstructor(squareSize))
			require.NoError(t, err)
...
func Test_ConvertEDStoShares(t *testing.T) {
	squareWidth := 16
	shares := share.RandShares(squareWidth * squareWidth)
	...
	require.Equal(t, share.ToBytes(shares), resshares)
}
...
func TestGetSharesByNamespace(t *testing.T) {
	...
	tests := []struct {
		...
		{rawData: share.RandShares(4)},
		{rawData: share.RandShares(16)},
	}
...
func TestCollectLeavesByNamespace_MultipleRowsContainingSameNamespaceId(t *testing.T) {
	...
	for _, row := range rowRoots {
		...
		for _, node := range leaves {
			...
			sh, err := share.NewShare(node.RawData()[share.NamespaceSize:])
			require.NoError(t, err)
...
func TestBatchSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			shrs, err := share.FromBytes(randEds.FlattenedODS())
			require.NoError(t, err)
...
func assertNoRowContainsNID(
	...
	for _, rowRoot := range rowRoots {
		...
		foundAbsenceRows++
		verified := data.Proof().VerifyNamespace(square.NewSHA256Hasher(), namespace.Bytes(), nil, rowRoot)
		require.True(t, verified)
	}
...
func randomNamespaces(total int) ([]share.Namespace, error) {
	...
	for i := range namespaces {
		namespaces[i] = share.RandomNamespace()
	}
