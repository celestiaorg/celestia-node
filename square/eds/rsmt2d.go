...
func (eds *Rsmt2D) DataHash(context.Context) (square.DataHash, error) {
	roots, err := square.NewAxisRoots(eds.ExtendedDataSquare)
	if err != nil {
		return square.DataHash{}, fmt.Errorf("while creating data root: %w", err)
	}
	return roots.Hash(), nil
}

// AxisRoots returns AxisRoots of the Accessor.
func (eds *Rsmt2D) AxisRoots(context.Context) (*square.AxisRoots, error) {
	roots, err := square.NewAxisRoots(eds.ExtendedDataSquare)
	if err != nil {
...
func (eds *Rsmt2D) Shares(_ context.Context) ([]share.Share, error) {
	return share.FromBytes(eds.ExtendedDataSquare.FlattenedODS())
}
...
func Rsmt2DFromShares(shares []share.Share, odsSize int) (*Rsmt2D, error) {
	...
	eds, err := rsmt2d.ComputeExtendedDataSquare(share.ToBytes(shares), square.DefaultRSMT2DCodec(), treeFn)
	if err != nil {
...
func getAxis(eds *rsmt2d.ExtendedDataSquare, axisType rsmt2d.Axis, axisIdx int) ([]share.Share, error) {
	switch axisType {
	case rsmt2d.Row:
		sh, err := share.FromBytes(eds.Row(uint(axisIdx)))
		return sh, err
	case rsmt2d.Col:
		sh, err := share.FromBytes(eds.Col(uint(axisIdx)))
		return sh, err
	default:
		panic("unknown axis")
	}
