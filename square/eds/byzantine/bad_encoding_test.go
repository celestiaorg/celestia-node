...
func TestBEFP_Validate(t *testing.T) {
	...
	roots, err := square.NewAxisRoots(byzSquare)
	...
	test := []struct {
		...
		},
		{
			...
			prepareFn: func() error {
				...
				validRoots, err := square.NewAxisRoots(validSquare)
				require.NoError(t, err)
...
func TestIncorrectBadEncodingFraudProof(t *testing.T) {
	...
	squareSize := 8
	shares := share.RandShares(squareSize * squareSize)
...
func newNamespacedBlockService() *namespacedBlockService {
	...
	mhcore.Register(sha256NamespaceFlagged, func() hash.Hash {
		nh := nmt.NewNmtHasher(square.NewSHA256Hasher(), share.NamespaceSize, true)
		nh.Reset()
		return nh
	})
	...
	bs.prefix = &cid.Prefix{
		...
		MhLength: square.NewSHA256Hasher().Size() + 2*share.NamespaceSize,
	}
	return bs
}
