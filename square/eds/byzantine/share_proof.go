...
import (
	...
	pb "github.com/celestiaorg/celestia-node/square/eds/byzantine/pb"
...
func (s *ShareWithProof) Validate(roots *square.AxisRoots, axisType rsmt2d.Axis, axisIdx, shrIdx int) bool {
	var rootHash []byte
	switch axisType {
	case rsmt2d.Row:
...
func ProtoToShare(protoShares []*pb.Share) ([]*ShareWithProof, error) {
	...
	for i, protoSh := range protoShares {
		...
		sh, err := share.NewShare(protoSh.Data)
		if err != nil {
			return nil, err
		}
...
func rootHashForCoordinates(r *square.AxisRoots, axisType rsmt2d.Axis, x, y int) []byte {
