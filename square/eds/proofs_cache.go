...
type proofsCache struct {
	...
	dataHash atomic.Pointer[square.DataHash]
	// rootsCache caches the axis roots
	rootsCache atomic.Pointer[square.AxisRoots]
...
func (c *proofsCache) DataHash(ctx context.Context) (square.DataHash, error) {
...
func (c *proofsCache) AxisRoots(ctx context.Context) (*square.AxisRoots, error) {
...
func (c *proofsCache) getShare(rowIdx, colIdx int) ([]byte, error) {
	...
	if colIdx > odsSize == half.IsParity {
		...
		return half.Shares[colIdx].ToBytes(), nil
	}
	...
	return shares[colIdx].ToBytes(), nil
}
