...
func testEDSes(t *testing.T, sizes ...int) map[string]*rsmt2d.ExtendedDataSquare {
	...
	testEDSes["EmptyODS"] = square.EmptyEDS()
	return testEDSes
}
...
func testAccessorDataHash(
	...
	expected, err := square.NewAxisRoots(eds)
	require.NoError(t, err)

	datahash, err := acc.DataHash(ctx)
	require.NoError(t, err)
	require.Equal(t, square.DataHash(expected.Hash()), datahash)
}
...
func testAccessorRowNamespaceData(
	...
	t.Run("not included", func(t *testing.T) {
		...
		for i, root := range roots.RowRoots[:odsSize] {
			...
			ns, err := share.NewNamespaceFromBytes(maxNs)
			require.NoError(t, err)

			absentNs, err := share.AddInt(ns, -1)
			require.NoError(t, err)
...
func testAccessorShares(
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			...
			sh, err := share.FromBytes(expected)
			require.NoError(t, err)
