...
// ShareAvailability implements square.Availability using Data Availability Sampling technique.
...
func (la *ShareAvailability) SharesAvailable(ctx context.Context, header *header.ExtendedHeader) error {
	...
	if square.DataHash(dah.Hash()).IsEmptyEDS() {
		return nil
	}
	...
	if len(failedSamples) > 0 {
		...
		return square.ErrNotAvailable
	}
	return nil
}

func rootKey(root *square.AxisRoots) datastore.Key {
