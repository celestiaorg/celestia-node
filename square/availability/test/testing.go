...
func RandFillBS(t *testing.T, n int, bServ blockservice.BlockService) *square.AxisRoots {
	shares := share.RandShares(n * n)
	return FillBS(t, bServ, shares)
}

// FillBS fills the given BlockService with the given shares.
func FillBS(t *testing.T, bServ blockservice.BlockService, shares []share.Share) *square.AxisRoots {
