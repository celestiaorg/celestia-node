...
func (s Sample) Verify(roots *square.AxisRoots, rowIdx, colIdx int) error {
...
func (s Sample) verifyInclusion(roots *square.AxisRoots, rowIdx, colIdx int) bool {
	size := len(roots.RowRoots)
	namespace := inclusionNamespace(s.Share, rowIdx, colIdx, size)
	rootHash := square.RootHashForCoordinates(roots, s.ProofType, uint(rowIdx), uint(colIdx))
