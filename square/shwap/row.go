...
func (r Row) Shares() ([]share.Share, error) {
	...
	rowShares, err := square.DefaultRSMT2DCodec().Decode(share.ToBytes(shares))
	if err != nil {
		return nil, err
	}
	return share.FromBytes(rowShares)
}
...
// the side of the row.
func (r Row) Verify(roots *square.AxisRoots, idx int) error {
...
// given row index.
func (r Row) verifyInclusion(roots *square.AxisRoots, idx int) error {
	shrs, err := r.Shares()
	if err != nil {
