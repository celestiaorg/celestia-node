...
func TestShrexGetter(t *testing.T) {
	...
	t.Run("ND_namespace_not_included", func(t *testing.T) {
		...
		nID, err := share.AddInt(maxNamespace, -1)
		require.NoError(t, err)
		// check for namespace to be between max and min namespace in root
		require.Len(t, square.RowsWithNamespace(roots, nID), 1)
		...
		require.Len(t, square.RowsWithNamespace(roots, nID), 0)
	...
	t.Run("ND_namespace_not_in_dah", func(t *testing.T) {
		...
		namespace, err := share.AddInt(maxNamespace, 1)
		require.NoError(t, err)
		// check for namespace to be not in root
		require.Len(t, square.RowsWithNamespace(roots, namespace), 0)
...
func generateTestEDS(t *testing.T) (*rsmt2d.ExtendedDataSquare, *square.AxisRoots, share.Namespace) {
	eds := edstest.RandEDS(t, 4)
	roots, err := square.NewAxisRoots(eds)
	require.NoError(t, err)
	max := nmt.MaxNamespace(roots.RowRoots[(len(roots.RowRoots))/2-1], share.NamespaceSize)
	ns, err := share.NewNamespaceFromBytes(max)
	require.NoError(t, err)
	return eds, roots, ns
}
...
func addToNamespace(namespace share.Namespace, val int) (share.Namespace, error) {
	...
	result := make([]byte, namespace.Len())
	if val > 0 {
		binary.BigEndian.PutUint64(result[namespace.Len()-8:], uint64(val))
	} else {
		binary.BigEndian.PutUint64(result[namespace.Len()-8:], uint64(-val))
	}
	...
	if carry != 0 {
		return share.Namespace{}, errors.New("namespace overflow")
	}

	return share.NewNamespaceFromBytes(result)
}
