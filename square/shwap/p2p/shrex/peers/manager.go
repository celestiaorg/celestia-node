...
func (m *Manager) Peer(ctx context.Context, datahash square.DataHash, height uint64,
...
func (m *Manager) doneFunc(datahash square.DataHash, peerID peer.ID, source peerSource) DoneFunc {
...
func (m *Manager) isBlacklistedHash(hash square.DataHash) bool {
	m.lock.Lock()
	defer m.lock.Unlock()
