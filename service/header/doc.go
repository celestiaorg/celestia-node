...
up into an ExtendedHeader and broadcast it to the headersub gossipsub topic. All nodes subscribed
...
There are 5 main components in the header package:
	1. CoreListener listens for new blocks from the celestia-core network (run by bridge nodes only),
	   extends them, generates a new ExtendedHeader and publishes it to the headersub gossipsub topic.
	2. P2PSubscriber listens for new ExtendedHeaders from the celestia data availability (DA) network (via
	   the headersub gossipsub topic)
	...
	4. Syncer manages syncing past ExtendedHeaders from either the DA network or a celestia-core
...
For bridge nodes, the general flow of the header Service is as follows:
	...
	3. CoreListener publishes the new ExtendedHeader to headersub, notifying all subscribed peers of the new
	   ExtendedHeader
	4. Syncer is already subscribed to the headersub topic so it receives new ExtendedHeaders locally from
...
For full and light nodes, the general flow of the header Service is as follows:
	...
	3. While the Syncer is requesting headers between local head and network head, it appends each subsequent
	   header to the chain via Store as the new local head.
*/
package header
