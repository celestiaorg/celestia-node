...
type heightSub struct {
	...
	heightReqs   map[uint64][]chan *extheader.ExtendedHeader
}

// newHeightSub instantiates new heightSub.
func newHeightSub() *heightSub {
	return &heightSub{
		heightReqs: make(map[uint64][]chan *extheader.ExtendedHeader),
	}
}
...
func (hs *heightSub) Sub(ctx context.Context, height uint64) (*extheader.ExtendedHeader, error) {
	if hs.Height() >= height {
	...
	resp := make(chan *extheader.ExtendedHeader, 1)
...
func (hs *heightSub) Pub(headers ...*extheader.ExtendedHeader) {
	height := hs.Height()
