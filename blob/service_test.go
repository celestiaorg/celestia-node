		...
		},
		{
			...
			expectedResult: func(res any, err error) {
				...
				for i := range blobs {
					...
					shareOffset += libshare.SparseSharesNeededV2(uint32(len(blobs[i].Data())), blobs[i].Signer() != nil)
				}
			},
		},
		{
...
func TestService_Get(t *testing.T) {
	...
	for i, blob := range blobs {
		...
		shareOffset += libshare.SparseSharesNeededV2(uint32(len(blob.Data())), blob.Signer() != nil)
	}
}
