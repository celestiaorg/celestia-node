...
func (p *parser) set(index int, shrs []gosquare.Share) ([]gosquare.Share, error) {
	if len(shrs) == 0 {
	...
	p.length = gosquare.SparseSharesNeeded(length)
	return shrs, nil
}
...
func (p *parser) addShares(shares []gosquare.Share) (shrs []gosquare.Share, isComplete bool) {
	index := -1
...
func (p *parser) parse() (*Blob, error) {
	...
	blobs, err := gosquare.ParseBlobs(p.shares)
	if err != nil {
		return nil, err
	}
...
func (p *parser) skipPadding(shares []gosquare.Share) ([]gosquare.Share, error) {
	if len(shares) == 0 {
		return nil, errEmptyShares
	}
