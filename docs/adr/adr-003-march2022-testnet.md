...
If fraud is detected, the **full** node will generate the proof and propagate it to the `FraudSub` gossip network. 
If no fraud is detected, the **full** node will continue operations without propagating any messages to the network,
but will still continue listening for messages from `FraudSub`.

If a fraud proof is received via `FraudSub`, **full** nodes will perform the verification themselves to check to ensure
the fraud proof is valid. 
If the fraud proof is valid, the node should immediately halt all operations.
If it is invalid, the node proceeds operations as usual. 
