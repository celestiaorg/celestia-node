...
func (rs *ranges) Add(h *header.ExtendedHeader) {
	head := rs.Head()
...
func (r *headerRange) Append(h ...*header.ExtendedHeader) {
	r.lk.Lock()
...
func (r *headerRange) Before(end uint64) ([]*header.ExtendedHeader, uint64) {
	r.lk.Lock()
	defer r.lk.Unlock()
