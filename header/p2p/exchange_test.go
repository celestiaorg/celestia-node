...
func TestExchange_RequestFullRangeHeaders(t *testing.T) {
	...
	for index := range servers {
		servers[index], err = NewExchangeServer(hosts[index], store, WithProtocolSuffix[ServerParameters]("private"))
		require.NoError(t, err)
...
func TestExchange_RequestByHash(t *testing.T) {
	...
	serv, err := NewExchangeServer(host, store, WithProtocolSuffix[ServerParameters]("private"))
	require.NoError(t, err)
	err = serv.Start(ctx)
	require.NoError(t, err)
	t.Cleanup(func() {
...
func TestExchange_RequestByHashFails(t *testing.T) {
	...
	// get host and peer
	host, peer := net.Hosts()[0], net.Hosts()[1]
	serv, err := NewExchangeServer(host, headerMock.NewStore(t, 0), WithProtocolSuffix[ServerParameters]("private"))
	require.NoError(t, err)
	err = serv.Start(ctx)
	require.NoError(t, err)
	t.Cleanup(func() {
...
func createP2PExAndServer(t *testing.T, host, tpeer libhost.Host) (*Exchange, *headerMock.MockStore) {
	store := headerMock.NewStore(t, 5)
	serverSideEx, err := NewExchangeServer(tpeer, store, WithProtocolSuffix[ServerParameters]("private"))
	require.NoError(t, err)
	err = serverSideEx.Start(context.Background())
	require.NoError(t, err)
	connGater, err := conngater.NewBasicConnectionGater(sync.MutexWrap(datastore.NewMapDatastore()))
	require.NoError(t, err)
	ex, err := NewExchange(host, []peer.ID{tpeer.ID()}, connGater, WithProtocolSuffix[ClientParameters]("private"))
	require.NoError(t, err)
