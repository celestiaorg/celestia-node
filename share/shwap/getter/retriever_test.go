...
func TestRetriever_Retrieve(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			// generate EDS
			eds := edstest.RandEDS(t, tc.squareSize)
			height := height.Add(1)
			put(t, store, eds, height)
			...
			hdr := &header.ExtendedHeader{RawHeader: header.RawHeader{Height: int64(height)}, DAH: root}

			out, err := r.Retrieve(ctx, hdr)
			require.NoError(t, err)
