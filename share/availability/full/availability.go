...
func (fa *ShareAvailability) Prune(ctx context.Context, eh *header.ExtendedHeader) error {
	height := eh.Height()
	if fa.archival {
		log.Debugf("trimming Q4 from block at height %d", height)
		return fa.store.RemoveQ4(ctx, height)
	}

	log.Debugf("removing block %s at height %d", height)
	return fa.store.RemoveODSQ4(ctx, height)
}
