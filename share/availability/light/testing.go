...
func randLightServiceWithSquare(t *testing.T, n int) (*availability.Service, *share.Root) {
	bServ := mdutils.Bserv()

	return availability.NewService(bServ, TestLightAvailability(bServ)), availability_test.RandFillBS(t, n, bServ)
}
...
func randLightService() (*availability.Service, blockservice.BlockService) {
	bServ := mdutils.Bserv()
	return availability.NewService(bServ, TestLightAvailability(bServ)), bServ
}
...
func Node(dn *availability_test.DagNet) *availability_test.Node {
	nd := dn.Node()
	nd.Service = availability.NewService(nd.BlockService, TestLightAvailability(nd.BlockService))
	return nd
}
