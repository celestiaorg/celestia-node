...
var (
	...
	token   = flag.String("token", "", "RPC token to use for authentication")
)
...
func parseFlags() (cliArgs, error) {
	...
	if *rpcURL == "" {
		return cliArgs{}, fmt.Errorf("--rpc-url is required")
	}

	return cliArgs{
		rpcURL:     *rpcURL,
		skipGetRow: *skipGetRow,
		timeout:    *timeout,
		token:      *token,
	}, nil
}
...
func runMain(args cliArgs) error {
	ctx, cancel := context.WithTimeout(context.Background(), args.timeout)
	defer cancel()

	serverAddr := args.rpcURL
	if !strings.HasPrefix(serverAddr, "http://") && !strings.HasPrefix(serverAddr, "https://") {
		serverAddr = "http://" + serverAddr
	}

	client, err := rpcclient.NewClient(ctx, serverAddr, args.token)
	if err != nil {
